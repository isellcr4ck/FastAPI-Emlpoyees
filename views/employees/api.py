...
def get_employee_by_id(employee_id: int) -> Employee:
    employee = storage.get_employee_by_id(id = employee_id)
...
def create_employee(employee: EmployeeCreate) -> Employee:
    return storage.create_employee(employee_create=employee)

@router.delete("/{employee_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_employee(employee_id: int) -> None:
    return storage.remove_employee_by_id(id = employee_id)